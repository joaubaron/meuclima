...
// Textura lunar: crateras com borda clara e centro escuro
const cratersData = [
...
cratersData.forEach(([cx,cy,r]) => {
  // anel claro (borda da cratera)
  const cring = document.createElementNS(svgNS,'circle');
  cring.setAttribute('cx',cx);cring.setAttribute('cy',cy);cring.setAttribute('r',r);
  cring.setAttribute('fill','none');
  cring.setAttribute('stroke','#4a5070');
  cring.setAttribute('stroke-width','1.5');
  cring.setAttribute('clip-path','url(#mc)');
  svg.appendChild(cring);
  // centro escuro (profundidade)
  const cdark = document.createElementNS(svgNS,'circle');
  cdark.setAttribute('cx',cx);cdark.setAttribute('cy',cy);cdark.setAttribute('r',r-1.5);
  cdark.setAttribute('fill','#0a0a1a');
  cdark.setAttribute('opacity','0.7');
